define([
	...
	var UIOutLevelSystem = Ash.System.extend({
		...
		updateDespair: function () {
			...
			var isValidDespairThirst = GameGlobals.gameState.unlockedFeatures["resource_water"] && !this.hasAccessToResource(resourceNames.water, false, false);
			...
			var showDespair = GameGlobals.gameState.isFeatureUnlocked("camp") && !hasCampHere && !isFirstPosition && (isValidDespairHunger || isValidDespairThirst || isValidDespairStamina || isValidDespairMove);
		...
		getResourcesDescription: function (isScouted, featuresComponent, statusComponent) {
			...
			var description = "";
			let hasScavengeableResources = featuresComponent.resourcesScavengable.getTotal() > 0;
			let hasScavengeableItems = featuresComponent.itemsScavengeable.length > 0;
			let hasScavengeableContent = hasScavengeableResources || hasScavengeableItems;
			if (isScouted && GameGlobals.gameState.unlockedFeatures.scavenge && hasScavengeableContent) {
				description += "Resources remaining: " + UIConstants.roundValue(100 - statusComponent.getScavengedPercent()) + "%<br/>";
			}
		...
		updateLocales: function () {
			...
			if (numNewItems > 0) {
				let newRowsScope = "#table-out-actions-locales tr.locale-list-item-uninitialized";
				GameGlobals.buttonHelper.updateButtonDisabledStates("#table-out-actions-locales", true);
				GameGlobals.uiFunctions.registerActionButtonListeners(newRowsScope);
				GameGlobals.uiFunctions.generateButtonOverlays(newRowsScope);
				GameGlobals.uiFunctions.generateCallouts(newRowsScope);
				$(newRowsScope).removeClass("locale-list-item-uninitialized");
		...
		createLocaleListItem: function () {
			...
			li.$root = $("<tr class='locale-list-item locale-list-item-uninitialized'><td>" + button + "</td><td>" + info + "</td></tr>");
